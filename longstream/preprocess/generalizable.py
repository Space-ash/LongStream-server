...
def prepare_input_cfg(cfg: Dict) -> Dict:
    input_cfg = dict(cfg.get("input", {}))
    mode = str(input_cfg.get("mode", "prepared") or "prepared").lower()
    if mode == "prepared":
        return cfg

    source_path = input_cfg.get("source_path", None)
    prepared_root = input_cfg.get("prepared_root", None)
    scene_name = input_cfg.get("scene_name", "runtime_scene")
    camera_id = str(input_cfg.get("camera_id", "00"))
    image_ext = str(input_cfg.get("image_ext", "png")).lower()
    overwrite = bool(input_cfg.get("overwrite", True))
    recursive = bool(input_cfg.get("recursive_images", False))
    target_fps = input_cfg.get("target_fps", None)

    if not source_path:
        raise ValueError("input.source_path must be set when input.mode is video/images")
    if not prepared_root:
        raise ValueError(
            "input.prepared_root must be set when input.mode is video/images"
        )
    ...
    cfg.setdefault("data", {})
    cfg["data"]["img_path"] = prepared_root
    cfg["data"]["seq_list"] = [scene_name]
    cfg["data"]["camera"] = camera_id
    return cfg
