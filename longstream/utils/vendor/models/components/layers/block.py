...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, pos=None, attn_mask=None, kv_cache=None) -> Tensor:
        def attn_residual_func(
            x: Tensor, pos=None, attn_mask=None, kv_cache=None
        ) -> Tensor:
            if kv_cache is not None:
                x, kv_cache = self.attn(
                    self.norm1(x), pos=pos, attn_mask=attn_mask, kv_cache=kv_cache
                )
        ...
        if self.training and self.sample_drop_ratio > 0.1:
            assert (
                attn_mask is None and kv_cache is None
            ), "attn_mask and kv_cache are not supported for drop_add_residual_stochastic_depth yet"
            ...
            if kv_cache is not None:
                delta_x, kv_cache = attn_residual_func(
                    x, pos=pos, attn_mask=attn_mask, kv_cache=kv_cache
                )
            else:
                delta_x = attn_residual_func(x, pos=pos, attn_mask=attn_mask)

            x_after_attn = x + self.drop_path1(delta_x)

            x = x_after_attn + self.drop_path1(ffn_residual_func(x_after_attn))
        else:
            x_input = x
            ...
            x_after_attn = x + delta_x

            x = x_after_attn + ffn_residual_func(x_after_attn)
