...
from longstream.utils.vendor.croco.models.dpt_block import DPTOutputAdapter
from longstream.utils.vendor.croco.models.blocks import Mlp

import longstream.utils.vendor.dust3r.utils.path_to_croco
...
class PixelwiseTaskWithDPT(nn.Module):
    ...
    def forward(self, x, img_info):
        ...
        if self.postprocess:
            out = self.postprocess(out, self.depth_mode, self.conf_mode, self.vis_mode)
        return out
...
class Cat_MLP_LocalFeatures_DPT_Pts3d(PixelwiseTaskWithDPT):
    ...
    def __init__(
        ...
        **kwargs
        ...
        patch_size = desc_head_args["patch_size"]
        ...
        self.desc_mode = desc_head_args["desc_mode"]
        self.two_confs = desc_head_args["two_confs"]
        self.desc_conf_mode = desc_head_args["desc_conf_mode"]
        idim = desc_head_args["enc_embed_dim"] + desc_head_args["dec_embed_dim"]

        self.features_head = Mlp(
            in_features=idim,
    ...
    def forward(self, decout, img_shape):
        ...
        local_features = self.features_head(cat_output)
        local_features = local_features.transpose(-1, -2).view(
            B, -1, H // self.patch_size, W // self.patch_size
        )
        local_features = F.pixel_shuffle(local_features, self.patch_size)
