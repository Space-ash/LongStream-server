import torch
from longstream.utils.vendor.croco.models.blocks import PatchEmbed
...
class ManyAR_PatchEmbed(PatchEmbed):
    ...
    def forward(self, img, true_shape):
        ...
        if is_landscape.any():
            ...
            new_landscape_content = new_landscape_content.permute(0, 2, 3, 1).flatten(
                1, 2
            )
        if is_portrait.any():
            new_protrait_content = self.proj(img[is_portrait].swapaxes(-1, -2))
            new_protrait_content = new_protrait_content.permute(0, 2, 3, 1).flatten(
                1, 2
            )

        x = img.new_empty(
            (B, n_tokens, self.embed_dim), dtype=next(self.named_parameters())[1].dtype
        )
        ...
        if is_landscape.any():
            pos[is_landscape] = self.position_getter(1, H, W, pos.device).expand(
                is_landscape.sum(), -1, -1
            )
        if is_portrait.any():
            pos[is_portrait] = self.position_getter(1, W, H, pos.device).expand(
                is_portrait.sum(), -1, -1
            )
